...
/**
 This is a class that deep copies the values from JS to C++. It does not convert
 back to JS values. This is used when reading values from JS that will never be
 sent back to the JS side but just handled in the C++ world.
 */
class JsiValue {
public:
  ...
  template <typename T> std::shared_ptr<T> tryGetAs() {
    if (_type != PropType::HostObject) {
      return nullptr;
    }
    return std::dynamic_pointer_cast<T>(_hostObject);
  ...
  const jsi::HostFunctionType getAsFunction() { return getAsHostFunction(); }
...
protected:
  const std::unordered_map<PropId, std::shared_ptr<JsiValue>> &
  getProps() const {
    return _props;
  }
...
private:
  ...
  void setArray(jsi::Runtime &runtime, const jsi::Object &obj) {
    ...
    for (size_t i = 0; i < size; ++i) {
      auto p =
          std::make_shared<JsiValue>(runtime, arr.getValueAtIndex(runtime, i));
      _array.push_back(p);
    }
  }
