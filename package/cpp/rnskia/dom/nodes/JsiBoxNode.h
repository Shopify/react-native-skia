...
protected:
  void renderNode(DrawingContext* context) override {
    ...
    auto box = _rrectProp->isSet() ? *_rrectProp->getDerivedValue() :
    ...
    for (auto &shadow: shadows) {
      if (shadow->getBoxShadowProps()->isInner()) {
        ...
        // Render!
        context->getCanvas()->drawDRRect(outer, inner, *shadow->getBoxShadowProps()->getDerivedValue());
  ...
  void defineProperties(NodePropsContainer* container) override {
    ...
    _rrectProp = container->defineProperty(std::make_shared<RRectProp>(JsiPropId::get("box")));
    _rectProp = container->defineProperty(std::make_shared<RectProp>(JsiPropId::get("box")));
  }
  
private:
  SkRRect inflate(const SkRRect &box, SkScalar dx, SkScalar dy, size_t tx = 0, size_t ty = 0) {
    return SkRRect::MakeRectXY(
                               ...
                               box.getSimpleRadii().x() + dx,
                               box.getSimpleRadii().y() + dy
                               );
  }
  
  SkRRect deflate(const SkRRect &box, SkScalar dx, SkScalar dy, size_t tx = 0, size_t ty = 0) {
