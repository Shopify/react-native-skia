...
class JsiBackdropFilterNode :
public JsiDomDrawingNode,
...
protected:
  void draw(DrawingContext* context) override {
    if (getChildren().size() == 0) {
      throw std::runtime_error("Expected at least one child in the BackdropFilter node.");
    }
    ...
    auto filter = colorFilter != nullptr ?
      SkImageFilters::ColorFilter(colorFilter->getCurrent(),
                                  nullptr) : imageFilter->getCurrent();
    
    canvas->saveLayer(SkCanvas::SaveLayerRec(nullptr,
                                             nullptr,
                                             filter.get(),
                                             0));
    canvas->restore();
