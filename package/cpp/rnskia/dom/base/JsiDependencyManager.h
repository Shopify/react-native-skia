...
public:
  ...
  /**
   ...
   properties have changed. NOTE: In this implementation we call unsubscribe directly without going
   through the JSI method - but the method is kept for compatibility with the JS implementation
   */
  ...
  /**
   Adds listeners to the provided values so that the node is notified when a value changes.
   */
  JSI_HOST_FUNCTION(subscribeNode) {
    ...
    for (const auto &propMapping: node->getPropsContainer()->getMappedProperties()) {
      ...
      if (isAnimatedValue(nativeValue)) {
        ...
        auto animatedValue = std::dynamic_pointer_cast<RNSkReadonlyValue>(nativeValue.getValue(PropNameValue)->getAsHostObject());
  ...
  JSI_HOST_FUNCTION(update) {
    if (_unregisterValues != nullptr) {
      ...
      _unregisterValues->asFunction(runtime).call(runtime, jsi::Value::undefined(), 0);
    ...
    _unregisterValues = std::make_shared<jsi::Object>(func.call(runtime, array, 1).asObject(runtime));
  ...
  */
  ...
  createCtor(std::shared_ptr <RNSkPlatformContext> context) {
    return JSI_HOST_FUNCTION_LAMBDA{
      // Params: registerValues: (values: Array<SkiaValue<unknown>>) => () => void
