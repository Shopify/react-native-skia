...
class PointModeProp:
public DerivedProp<SkCanvas::PointMode> {
public:
  ...
  void updateDerivedValue() override {
    if (_pointModeProp->isSet()) {
      setDerivedValue(getPointModeFromStringValue(_pointModeProp->value()->getAsString()));
    } else {
...
private:
  static SkCanvas::PointMode getPointModeFromStringValue(const std::string& value) {
    if (value == "points") {
    ...
    throw std::runtime_error("Expected valid point mode for Points node, got \"" + value + "\".");
  }
