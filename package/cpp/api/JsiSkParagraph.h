...
public:
  ...
  JSI_HOST_FUNCTION(getRectsForRange) {
    ...
    auto result = _paragraph->getRectsForRange(start, end, RectHeightStyle::kTight, RectWidthStyle::kTight);
    auto returnValue = jsi::Array(runtime, result.size());
    for (size_t i=0; i < result.size(); ++i) {
      returnValue.setValueAtIndex(runtime, i, JsiSkRect::toValue(runtime, getContext(), result[i].rect));
    }
    return returnValue;
  }
  
  JSI_HOST_FUNCTION(getLineNumberAt) {
    auto index = static_cast<size_t>(getArgumentAsNumber(runtime, arguments, count, 0));
  ...
  JSI_HOST_FUNCTION(getLineMetrics) {
    ...
    for (size_t i=0; i < metrics.size(); ++i) {
      returnValue.setValueAtIndex(runtime, i, JsiSkRect::toValue(runtime, getContext(), SkRect::MakeXYWH(metrics[i].fLeft, height, metrics[i].fWidth, metrics[i].fHeight)));
  ...
  JSI_HOST_FUNCTION(getPath) {
    ...
    return jsi::Object::createFromHostObject(runtime, std::make_shared<JsiSkPath>(getContext(), std::move(paths)));
  }
  ...
  JSI_EXPORT_FUNCTIONS(JSI_EXPORT_FUNC(JsiSkParagraph, layout),
                       ...
                       JSI_EXPORT_FUNC(JsiSkParagraph, getGlyphPositionAtCoordinate),
