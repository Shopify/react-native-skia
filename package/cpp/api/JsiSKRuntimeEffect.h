  ...
  JsiSkRuntimeEffect(RNSkPlatformContext *context, sk_sp<SkRuntimeEffect> rt)
    ...
    installFunction(
        "makeShader", JSI_FUNC_SIGNATURE {
          ...
          auto shader =
              getObject()->makeShader(std::move(uniforms), children.data(),
    ...
    installFunction(
        "makeShaderWithChildren", JSI_FUNC_SIGNATURE {
          // Set up arguments - start with Uniforms
          sk_sp<SkData> uniforms;
          if (count >= 1) {
            // Verify uniforms
          ...
          auto isOpaque =
              arguments[1].isBool() ? arguments[1].getBool() : false;
          // Children
          std::vector<sk_sp<SkShader>> children;
    ...
    installFunction(
        "getUniformCount", JSI_FUNC_SIGNATURE {
          return jsi::Value(getJsNumber(getObject()->uniformSize()));
        });

    installFunction(
        "getUniformFloatCount", JSI_FUNC_SIGNATURE {
          return jsi::Value(
              getJsNumber(getObject()->uniformSize() / sizeof(float)));
        });

    installFunction(
        "getUniformName", JSI_FUNC_SIGNATURE {
