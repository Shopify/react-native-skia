...
public:
  ...
  JSI_HOST_FUNCTION(TokenizeText) {
	...
	for (size_t i = 0; i < wordsUtf16.size(); i++) {
	  wordsArray.setValueAtIndex(runtime, i, jsi::Value(static_cast<int>(wordsUtf16[i])));
	}
	  
	jsi::Array graphemesArray(runtime, graphemesUtf16.size());
    for (size_t i = 0; i < graphemesUtf16.size(); i++) {
	  graphemesArray.setValueAtIndex(runtime, i, jsi::Value(static_cast<int>(graphemesUtf16[i])));
	}
    ...
    for (size_t i = 0; i < lineBreaksUtf16.size(); i++) {
        ...
        breakArray.setValueAtIndex(runtime, 0, jsi::Value(static_cast<int>(lineBreaksUtf16[i].pos)));
        breakArray.setValueAtIndex(runtime, 1, jsi::Value(static_cast<int>(lineBreaksUtf16[i].breakType)));
  ...
  JSI_EXPORT_FUNCTIONS(JSI_EXPORT_FUNC(JsiSkParagraphFactory,
                                       RequiresClientICU),
                       JSI_EXPORT_FUNC(JsiSkParagraphFactory,
                                       TokenizeText))
