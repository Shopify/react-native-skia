...
public:
    ...
    bool onAnimate(double nanos) {
        fDegrees = nanos * 1e-9 * 360.f/10.f + 45.f;
        // prime number periods, like locusts.
        fColor.fR = SineWave(nanos, 13.f, 0.f, 0.f, 1.f);
        fColor.fG = SineWave(nanos, 23.f, 5.f, 0.f, 1.f);
        fColor.fB = SineWave(nanos, 11.f, 0.f, 0.f, 1.f);
        fColor.fA = 1.f;
        return true;
    }
...
private:
    ...
    static inline float SineWave(double time,
        ...
        double t = time * 1e-9 + phaseInSecs;
