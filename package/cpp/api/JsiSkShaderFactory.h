...
public:
  JsiSkShaderFactory(RNSkPlatformContext *context) : JsiSkHostObject(context) {
    installFunction(
        "MakeLinearGradient", JSI_FUNC_SIGNATURE {
          auto p1 =
              *JsiSkPoint::fromValue(runtime, arguments[0].asObject(runtime))
                   .get();
          auto p2 =
              *JsiSkPoint::fromValue(runtime, arguments[1].asObject(runtime))
                   .get();
          SkPoint pts[] = {p1, p2};
          ...
          if (count == 5) {
            ...
            gradient = SkGradientShader::MakeLinear(
                pts, colors.data(), positions.data(), (int)size, tileMode, flag,
                localMatrix);
          }
          return jsi::Object::createFromHostObject(
              runtime, std::make_shared<JsiSkShader>(context, gradient));
        });

    installFunction(
        "MakeRadialGradient", JSI_FUNC_SIGNATURE {
          auto center =
              *JsiSkPoint::fromValue(runtime, arguments[0].asObject(runtime))
                   .get();
    ...
    installFunction(
        "MakeSweepGradient", JSI_FUNC_SIGNATURE {
          ...
          sk_sp<SkShader> gradient = SkGradientShader::MakeSweep(
              x, y, colors.data(), positions.data(), (int)size, tileMode,
    ...
    installFunction(
        "MakeTwoPointConicalGradient", JSI_FUNC_SIGNATURE {
          auto start =
              *JsiSkPoint::fromValue(runtime, arguments[0].asObject(runtime))
                   .get();
          ...
          auto end =
              *JsiSkPoint::fromValue(runtime, arguments[2].asObject(runtime))
                   .get();
          ...
          if (count == 7) {
            gradient = SkGradientShader::MakeTwoPointConical(
                ...
                positions.data(), (int)size, tileMode);
          } else {
            auto localMatrix =
            ...
            gradient = SkGradientShader::MakeTwoPointConical(
                start, startRadius, end, endRadius, colors.data(),
                positions.data(), (int)size, tileMode, flag, localMatrix);
          }
          return jsi::Object::createFromHostObject(
              runtime, std::make_shared<JsiSkShader>(context, gradient));
        });
    ...
    installFunction(
        "MakeTurbulence", JSI_FUNC_SIGNATURE {
    ...
    installFunction(
        "MakeFractalNoise", JSI_FUNC_SIGNATURE {
          auto baseFreqX = arguments[0].asNumber();
          auto baseFreqY = arguments[1].asNumber();
          auto octaves = arguments[2].asNumber();
          auto seed = arguments[3].asNumber();
