    ...
    JsiSkCanvas(RNSkPlatformContext *context) : JsiSkHostObject(context) {
        ...
        installFunction(
            "drawImage", JSI_FUNC_SIGNATURE {
                ...
                _canvas->drawImage(
                    image, x, y, SkSamplingOptions(), paint.get());
                return jsi::Value::undefined();
            });
        ...
        installFunction(
            "drawImageRect", JSI_FUNC_SIGNATURE {
                ...
                _canvas->drawImageRect(
                    image, *rect, SkSamplingOptions(), paint.get());
                return jsi::Value::undefined();
            });
        ...
        installFunction(
            "drawPoints", JSI_FUNC_SIGNATURE {
                ...
                auto jsiPoints =
                        arguments[1].asObject(runtime).asArray(runtime);
                ...
                _canvas->drawPoints((SkCanvas::PointMode)pointMode, pointsSize, points.data(), *paint);
        ...
        installFunction(
            "saveLayer", JSI_FUNC_SIGNATURE {
                SkPaint* paint = arguments[0].isUndefined() ? nullptr : JsiSkPaint::fromValue(runtime, arguments[0]).get();
                SkRect* bounds = arguments[1].isNull() || arguments[1].isUndefined() ? nullptr : JsiSkRect::fromValue(runtime, arguments[1]).get();
                SkImageFilter* backdrop =  arguments[2].isNull() || count < 3 ? nullptr : JsiSkImageFilter::fromValue(runtime, arguments[2]).get();
                SkCanvas::SaveLayerFlags flags = count < 4 ? 0 : arguments[3].asNumber();
                return jsi::Value(_canvas->saveLayer(SkCanvas::SaveLayerRec(bounds, paint, backdrop, flags)));
            });
