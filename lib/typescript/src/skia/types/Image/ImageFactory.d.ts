...
export interface ImageFactory {
    ...
    /**
     * Returns an image with encoded pixel data from the given URL
     *
     * @param url - the url to fetch
     */
    MakeImageFromURL: (url: string) => Promise<SkImage | null>

    /**
     * Returns an image that is texture backed and available accross all threads with GrContext available (JS and UI thread at the moment).
     * @param data - Data object with bytes of data
     * @returns
     */
    MakeCrossContextTextureFromEncoded: (data: SkData) => SkImage | null;
}
