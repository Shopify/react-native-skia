...
public:
    ...
    void savePaint() {
        paints.push(std::make_shared<SkPaint>(*getPaint()));
    }
    ...
    void saveBackdropFilter() {
        ...
        sk_sp<SkImageFilter> imgf = imageFilters.pop();
        if (imgf) {
            imageFilter = imgf;
        } else {
            sk_sp<SkColorFilter> cf = colorFilters.pop();
            if (cf) {
    ...
    void materializePaint() {
        ...
        if (colorFilters.size() > 0) {
            getPaint()->setColorFilter(
                std::accumulate(colorFilters.begin(), colorFilters.end(), nullptr,
        ...
        if (imageFilters.size() > 0) {
            getPaint()->setImageFilter(
                std::accumulate(imageFilters.begin(), imageFilters.end(), nullptr,
        ...
        if (pathEffects.size() > 0) {
            getPaint()->setPathEffect(
                std::accumulate(pathEffects.begin(), pathEffects.end(), nullptr,
