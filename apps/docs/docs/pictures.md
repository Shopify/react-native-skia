...
This is great for animating property values with near-zero performance cost. However, if you need to execute a **variable number of drawing commands** each frame, this is where you need to use Pictures—React Native Skia's [immediate mode](/docs/canvas/rendering-modes#immediate-mode) API.
