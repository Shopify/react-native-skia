...
```
If you canvas is static and doesn't contain animation values, you can use the `__destroyGLContextAfterRender={true}` prop on your Canvas components to destroy the WebGL context after rendering.
...
export default function App() {
  return (
    <View style={{ flex: 1 }}>
      {
        // 20 Skia Canvases with __destroyGLContextAfterRender={true}
        new Array(20).fill(0).map((_, i) => (
          <Canvas key={i} style={{ width: 100, height: 100 }} __destroyGLContextAfterRender={true}>
