...
async function extractTwoslashSnippets() {
  ...
  const files = findMarkdownFiles(docsDir)
    .map(file => file.substring(docsPath.length + 1));
...
extractTwoslashSnippets()
  .then((results) => {
    console.log(`Found ${results.reduce((total, file) => total + file.snippets.length, 0)} twoslash snippets in ${results.length} files:`);
