...
For instance, if you've created `index.tsx` and `index.web.tsx` in the `app/` directory, update your `package.json` accordingly:
```patch
...
For the `app/index.tsx` file, directly invoke `renderRootComponent(App)`.
