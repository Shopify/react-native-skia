...
* With `useLoadSkiaWeb()`: using hooks to conditionally render the components that import Skia.
...
### Using Hooks to Conditionally Render
We provide a `useLoadSkiaWeb()` hook which allows you to conditionally render your app or components that depend on Skia.
This approach let you decide what to do while Skia is being loaded, such as render loading screens.
In the example below, we load Skia in our app's root file before rendering the our content.

```tsx
import { LoadSkiaWeb } from "@shopify/react-native-skia";

function App() {
  const [skiaLoaded, skiaError] = useLoadSkiaWeb();
  // You can also load other stuff here, such as fonts, remote data, etc.

  if (skiaError) {
    return <Text>Failed to load Skia</Text>;
  }
