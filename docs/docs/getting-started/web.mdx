...
We provide direct integrations with [React](#React), [Expo](#Expo), and [Remotion](#Remotion).
Below you will also find the manual installation steps to run the module on any React or React Native Web projects.
...
It is possible to use React Native Skia on the Web without the `react-native-web` dependency.
First, you need to pick your strategy to [Load Skia](#loading-skia).
Then, you will need to change the import path for the Skia components.
In the example below, we use [code splitting](#using-code-splitting) to load Skia and display our canvas.
...
```tsx
// Note that the import is not the same as in React Native 
import {Canvas, Circle, Group} from "@shopify/react-native-skia/lib/module/web/Canvas";
...
And below is an example using [defered component registration](#using-defered-component-registration).
...
LoadSkia().then(async () => {
    const App = await (await import("./HelloWorld")).default;
