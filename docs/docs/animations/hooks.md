...
This hook return a number indicating the time in milliseconds since the hooks was activated.
...
Paths need to be interpolatable, meaning they must contain the same number and types of commands. If the paths have different commands or different numbers of commands, the interpolation may not behave as expected. Ensure that all paths in outputRange are structured similarly for proper interpolation.
